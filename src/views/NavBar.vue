...
<template>
  <div class="justify-content-evenly align-items-center py-lg-4 fixed-bottom d-none d-sm-flex d-xs-flex" id="bottomNav">
    <div class="nav-item" v-for="(nav, index) in navs" :key="index"><RouterLink class="nav-link text-uppercase" :to="nav.loc">{{ nav.name }}</RouterLink></div>
  </div>
  <nav id="secondNav" class=".bottom-0 .position-fixed fixed-bottom">
    <div class="d-lg-none">
      <div class="container-fluid d-flex justify-content-between shadow pt-2">
        <RouterLink
          :to="nav.loc"
          class="text-center text-decoration-none textColor"
          v-for="(nav, index) in navs"
          v-bind:key="index"
        >
          <div class="mb-1">
            <span class="bg-primary rounded-5 p-1 text-white"><i :class="nav.icon"></i></span>
          </div>
          <div exact class="extraSmallFont pb-1">{{ nav.name }}</div>
        </RouterLink>
      </div>
    </div>
  </nav>
